...
from tkinter import filedialog, PhotoImage, CENTER
import googletrans
import os
from harvester.db import Database_Instance
from harvester.run_network_check import run_connection_check,offline_check
from backend.translate import TranslateText
from backend.voiceOutput import TextToSpeech
...
from backend.audio_list import load_audios, play_selected, delete_selected,open_folder_dialog
import getpass 
import threading
import requests
...
extract.xtract_images()
icons_credit = """<div>Icons made by <a href="https://www.freepik.com" title="Freepik">Freepik</a> from <a href="https://www.flaticon.com/" title="Flaticon">www.flaticon.com</a></div>"""

class AppLanguage:
    def __init__(self, **kwargs):
        self.theme = "darkly"
        self.app = ttk.Window(themename=self.theme)
        self.path1 = f"C:\\Users\\{getpass.getuser()}\\Pictures\\languages.png"
        self.path2 = f"C:\\Users\\{getpass.getuser()}\\Pictures\\languages.ico"
        self.pathx = f"C:\\Users\\{getpass.getuser()}\\Desktop\\LANGUAGE_TRANSLATOR_AUDIOS"
        
        # self.app.theme
        self.app.title("Ariko's Language App V7.30.24")
        self.icon_photo = PhotoImage(file=self.path1)#"./languages.png")
        self.app.iconphoto(False, self.icon_photo)

        # create tabs
        notebook = ttk.Notebook(self.app)
        notebook.pack(expand=True, fill="both")
        self.tab1 = ttk.Frame(notebook)  # name="Language Translator"
        self.tab2 = ttk.Frame(notebook)

        notebook.add(self.tab1, text="Languages Translator")
        notebook.add(self.tab2, text="Saved Audio Files")

        self.audio_files = ttk.Frame(self.tab2,width='300',height=60)
        self.audio_files.pack(side="top", padx=10, pady=10, fill="x")

        self.cta_audios = ttk.Frame(self.tab2,width='300',height=60)
        self.cta_audios.pack(side="bottom", padx=10, pady=10, fill="x",ipady=15)

        self.from_frame = ttk.Frame(self.tab1)
        self.from_frame.pack(
            padx=10,
            pady=8,
            fill="x",
        )

        self.text_boxes_frame = ttk.Frame(self.tab1)
        self.text_boxes_frame.pack(
            padx=10,
            pady=8,
            fill="x",
        )

        self.cta_buttons = ttk.Frame(self.tab1)
        self.cta_buttons.pack(
            padx=20,
            pady=8,
            ipadx=15,
            fill="x",
        ...
        for key, value in (googletrans.LANGCODES).items():
            self.languages1.append(f"{key},{value}")

        self.selected_lang1 = ttk.StringVar()
        self.selected_lang2 = ttk.StringVar()

        self.selected_lang1.set("English,en")
        self.selected_lang2.set("Russian,ru")

        self.load_widgets()
        load_audios(self.audio_listbox)
        
    def disable_buttons(self):
        self.translate_btn.configure(state='disabled')
        self.audio_btn.configure(state='disabled')
        
    def autospeaktranslation(self):
        self.tts_output

    def load_widgets(self):
        # add widgets on tabs

        self.var0 = ttk.IntVar()
        self.var1 = ttk.IntVar()
        self.var2 = ttk.IntVar()
        self.var3 = ttk.IntVar()
        self.var4 = ttk.IntVar()
        ...
        self.DarkMode.pack(side="left", padx=10, pady=8, fill="x")

        self.fromLanguage = tk.OptionMenu(
            self.from_frame,
            self.selected_lang1,
            *self.languages1,
            command=self.selectSource_language,
        )
        self.fromLanguage.pack(side="left", padx=10, pady=8, fill="x", ipadx=6, ipady=5)

        self.lang2 = ttk.Label(self.from_frame, text="Translate to")
        self.lang2.pack(side="left", padx=10, pady=8, fill="x")

        self.toLanguage = tk.OptionMenu(
            self.from_frame, self.selected_lang2, *self.languages1
        )
        self.toLanguage.pack(side="left", padx=10, pady=8, fill="x", ipadx=6, ipady=5)

        # right_side actions
        # INPUT ACTION BUTTONS
        self.autoDetect = ttk.Checkbutton(
            self.from_frame,
            text="Enable Auto Detect",
            variable=self.var1,
            command=self.enable_auto_detectLanguage,
        )
        self.autoDetect.pack(side="left", padx=10, pady=8, fill="x")

        self.enableAudio = ttk.Checkbutton(
            self.from_frame,
            text="Enable Voice Input",
            variable=self.var2,
            command=self.enable_audio_input,
            onvalue=1,
            offvalue=0,
        )
        self.enableAudio.pack(side="left", padx=10, pady=8, fill="x")

        self.saveAudioOutput = ttk.Checkbutton(
            self.from_frame,
            text="Save Audio Output",
            variable=self.var3,
            command=self.save_audio_output,
            onvalue=1,
            offvalue=0,
        )
        self.saveAudioOutput.pack(side="left", padx=10, pady=8, fill="x")

        # self.autospeakaftertranslation = ttk.Checkbutton(
        #     self.from_frame,
        #     text="Auto Speak Translation",
        #     variable=self.var4,
        #     command=self.autospeaktranslation,
        #     onvalue=1,
        #     offvalue=0,
        # )
        # self.autospeakaftertranslation.pack(side="left", padx=10, pady=8, fill="x")

        self.activationDuration = ttk.Label(
            self.from_frame,
            text=f"Lifetime Edition",
            foreground="#12890C",
            font=("Monserrat", 16),
        )
        self.activationDuration.pack(
            side="left", ipadx=5, ipady=5, padx=10, pady=10, fill="x"
        )
        ...
        self.textbox.pack(side="left", padx=10, pady=8, fill="x")

        self.textbox2 = ttk.Text(
            self.text_boxes_frame,
            relief="flat",
            font=("Arial", 16, "bold"),
            width=50,
        ...
        self.textbox2.pack(side="left", padx=10, pady=8, fill="x")

        image_source = self.path1#"./languages.png"
        self.icon = ImageTk.PhotoImage(Image.open(image_source))
        self.icon_display = ttk.Label(self.text_boxes_frame, text="", image=self.icon)
        self.icon_display.pack(side="left", padx=10, pady=8)

        # LoWER MOST ACTION BUTTONS
        opentxt_btn = ttk.Button(
            self.cta_buttons,
            bootstyle=(OUTLINE, INFO),
            text="Load Text File",
            command=self.load_text_file,
        )
        opentxt_btn.pack(side="left", padx=10, pady=8, ipadx=13, ipady=10, fill="x")

        self.translate_btn = ttk.Button(
            self.cta_buttons,
            text="TRANSLATE TEXT",
            bootstyle=(DARK),
            command=self.translate_textInput,
        )
        self.translate_btn.pack(side="left", padx=10, pady=8, ipadx=13, ipady=10, fill="x")

        clearInput_btn = ttk.Button(
            self.cta_buttons,
            text="Clear Input",
            bootstyle=(OUTLINE, DANGER),
            command=self.clear_input,
        )
        clearInput_btn.pack(side="left", padx=10, pady=8, ipadx=13, ipady=10, fill="x")
        ...
        self.version = ttk.Label(
            ...
            background="#ffffff",
            foreground="#000000",
            text="Developed by @Ariko 2024",
            font=("Arial", 12),
        )
        self.version.pack(pady=10, padx=10, ipady=6, ipadx=10, side="right", fill="x")

        save_btn = ttk.Button(
            self.cta_buttons,
            text="Save Output As text",
            bootstyle=(OUTLINE, INFO),
            command=self.save_translatedLang,
        )
        save_btn.pack(side="right", padx=10, pady=8, ipadx=13, ipady=10, fill="x")

        self.audio_btn = ttk.Button(
            self.cta_buttons,
            text="Speak Output",
            bootstyle=(DARK),
            command=self.tts_output,
        )
        self.audio_btn.pack(side="right", padx=10, pady=8, ipadx=13, ipady=10, fill="x")
        ...
        clear_btn.pack(side="right", padx=10, pady=8, ipadx=13, ipady=10, fill="x")

        saved_label = tk.Label(self.audio_files,text="Select saved audio files to play them",font=('Arial',14))
        saved_label.pack(ipady=10,padx=10,pady=10)

        self.audio_listbox = tk.Listbox(
            self.audio_files,height=20, width=300, font=("Arial", 14)
        )
        self.audio_listbox.pack(padx=10, pady=10, side="top", fill="x")
###########################################
        play_selected_audio = ttk.Button(
            self.cta_audios,
            text="Play Selected Audio",
            bootstyle=(OUTLINE, SUCCESS),
            command=lambda: play_selected(self.audio_listbox,self.selected_entry),
        )
        play_selected_audio.pack(
            side="left", padx=10, pady=8, ipadx=13, ipady=10
        )

        delete_selected_audio = ttk.Button(
            self.cta_audios,
            text="delete Selected Audio",
            bootstyle=(OUTLINE, DANGER),
            command=lambda: delete_selected(self.audio_listbox),
        )
        delete_selected_audio.pack(
            side="left", padx=10, pady=8, ipadx=13, ipady=10
        )

        self.selected_label = ttk.Label(self.cta_audios,text="Playing",font=('Arial',14))
        self.selected_label.pack(side="left", padx=10, pady=8, ipadx=13, ipady=10)

        self.selected_entry = ttk.Entry(self.cta_audios)
        self.selected_entry.pack(side="left",fill='x',expand=True, padx=10, pady=8, ipadx=13, ipady=10)
       
    def enable_auto_detectLanguage(self):
        print(self.autoDetect.state())
        if "selected" in self.autoDetect.state():
            print("Box checked")
        else:
            print("Unchecked")
    ...
    def enable_audio_input(self):
        start_new_thread(self.audioInputThread, ())
    ...
    def load_text_file(self):
        ...
        with open(self.file_path.name, "rb") as f:
            content = f.read().decode()
            self.textbox.insert("0.0", str(content) + "\n")
            f.close()

    def translate_textInput(self):
        # threading.Thread(target=self.validate,args=()).start()
        
        if "selected" in self.autoDetect.state():
            try:
                text = self.textbox.get("0.0", "end")
                self._from = translator.auto_detectText(text)
                self.selected_lang1.set(f"Detected,{self._from}")

                self._to = self.selected_lang2.get().split(",")[1]
                print(self._to)
                # if self.validate():
                resp = translator.translate_text(
                    self.textbox.get("0.0", "end"), self._from, self._to
                )
                # else:pass

            except Exception as e:
                self._to = self.selected_lang2.get().split(",")[1]  # strip(',')[-5:]
                resp = translator.translate_text(
                    self.textbox.get("0.0", "end"), self._from, self._to
                )
                print(e)
            ...
            try:
                self._from = self.selected_lang1.get().split(",")[1]
        ...
        self.textbox2.insert("0.0", str(resp))

    def clear_output(self):
        self.textbox2.delete("1.0", "end")

    def tts_output(self):
        self.tts_outputThread()
        

    def save_translatedLang(self):
        ...
        if _filePath:
            print(_filePath)
            data = self.textbox2.get("0.0", "end")
    ...
    def tts_outputThread(self):
        self.state = self.saveAudioOutput.state()

        try:
            if "selected" in self.state:
                tts.say_Savetext(
                    self.textbox2.get("0.0", "end"), self._to, self.selected_lang2.get()
                )
                
                tts.say_text(self.textbox2.get("0.0", "end"), self._to)
            ...
            msg.showerror(
                "Audio Not upported", f"There is no audio support for {self._to}"
            )
            print(e)
    


    def selectSource_language(self):
        pass

    def audioInputThread(self):
        self.state = self.enableAudio.state()

        if "selected" in self.state:
            auddata = voice.listenToAudio()
            self.textbox.delete("0.0", "end")
            self.textbox.insert("0.0", auddata)
            self.translate_textInput()
...
def run():
    while conn:
        connected = run_connection_check()
        print(connected)
        try:
            if connected:
...
if __name__ == "__main__":
    ...
    main_win = rum.mainloop()

   
        
    

extract.delete_images()
>>>>>>> 946caf4 (Fixed chinese language bug and added audio player)
